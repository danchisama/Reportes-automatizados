@echo off
set PROJECT_DIR=C:\Proyectos\Proyecto_Reportes_DC
...
.venv\Scripts\python.exe scripts\raw_report.py >> "%LOG_FILE%" 2>&1

:: 2. Procesar y generar reporte final
echo [%date% %time%] Generando reporte procesado... >> "%LOG_FILE%"
.venv\Scripts\python.exe scripts\processed_report.py >> "%LOG_FILE%" 2>&1
